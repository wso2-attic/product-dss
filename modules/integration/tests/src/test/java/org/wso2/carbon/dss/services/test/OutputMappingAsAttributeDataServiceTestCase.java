...
public class OutputMappingAsAttributeDataServiceTestCase extends DSSIntegrationTest {
    ...
    public void addOffice() throws AxisFault {
        ...
        serviceClient.sendRobust(getPayloadAddOffice("25", "NY", "Boston", "EMEA"), getServiceUrl(serviceName), "addOffice");
        serviceClient.sendRobust(getPayloadAddOffice("26", "NY", "Boston", null), getServiceUrl(serviceName), "addOffice");
        serviceClient.sendRobust(getPayloadAddOffice("27", "NY", "Boston", "NULL"), getServiceUrl(serviceName), "addOffice");
    }

    @Test(groups = {"wso2.dss"}, dependsOnMethods = {"addOffice"})
    public void getOffices() throws AxisFault {
        AxisServiceClient serviceClient = new AxisServiceClient();
        OMElement response = serviceClient.sendReceive(getPayloadGetOffice(), getServiceUrl(serviceName), "showAllOffices");
