...
public class NestedQueryTestCase extends DSSIntegrationTest {
    ...
    public void getCustomerOrdersNestedQuery() throws AxisFault {
        for (int i = 1; i < 6; i++) {
            getCustomerOrders();
        }
    ...
    @Test(groups = {"wso2.dss"})
    public void listOfficeNestedQueryOperation() throws AxisFault {
        for (int i = 1; i < 6; i++) {
            getOffices();
        }
    ...
    private void getCustomerName() throws AxisFault {
        ...
        OMElement result = new AxisServiceClient().sendReceive(payload, getServiceUrl(serviceName), "customerName");
    ...
    private void getCustomerOrders() throws AxisFault {
        OMElement payload = fac.createOMElement("customerOrders", omNs);

        OMElement result = new AxisServiceClient().sendReceive(payload, getServiceUrl(serviceName), "customerOrders");
        Assert.assertNotNull(result, "Response message null ");
        log.debug(result);
    ...
    private void getEmployeesInOffice() throws AxisFault {
        ...
        OMElement result = new AxisServiceClient().sendReceive(payload, getServiceUrl(serviceName), "employeesInOffice");
        Assert.assertNotNull(result, "Response message null ");
        log.debug(result);
    ...
    private void getOffices() throws AxisFault {
        OMElement payload = fac.createOMElement("listOffices", omNs);

        OMElement result = new AxisServiceClient().sendReceive(payload, getServiceUrl(serviceName), "listOffices");
        Assert.assertNotNull(result, "Response message null ");
        log.debug(result);
