...
public class SecureDataServiceTestCase extends DSSIntegrationTest {
    ...
    public void serviceDeployment() throws Exception {
        ...
        sqlFileLis.add(new File((ProductConstant.SYSTEM_TEST_RESOURCE_LOCATION + "artifacts"
    ...
    @Test(groups = {"wso2.dss"}, description = "Provides Authentication. Clients have Username Tokens")
    ...
    public void securityPolicy2() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            response = secureAxisServiceClient.sendReceive(userInfo.getUserName(), userInfo.getPassword(),
                                                           getServiceUrl(serviceName), "showAllOffices",
                                                           getPayload(), policyId);
            verifyResponce(response);
        }
    ...
    public void securityPolicy7() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            response = secureAxisServiceClient.sendReceive(userInfo.getUserName(), userInfo.getPassword(),
                                                           getServiceUrl(serviceName), "showAllOffices", getPayload(),
                                                           policyId);
            verifyResponce(response);
        }
    ...
    public void securityPolicy15() throws Exception {
        ...
        for (int i = 0; i < 5; i++) {
            response = secureAxisServiceClient.sendReceive(userInfo.getUserName(),
                                                           userInfo.getPassword(), getServiceUrl(serviceName),
